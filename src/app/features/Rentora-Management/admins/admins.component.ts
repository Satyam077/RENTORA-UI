...
export class AdminsComponent implements OnInit, AfterViewInit {
  users: User[] = [];
  isLoading = false;
  currentUserId: string = ''; // If needed for identification
  ...
  loadUsers(): void {
    ...
    this.usersService.getAllUsers().subscribe({
      next: (res: any) => {
        ...
        // Filter for Admins and SuperAdmins (or whatever logic was there)
        this.users = allUsers.filter(u => u.role === Role.Admin || u.role === Role.SuperAdmin);
  ...
  openAddDialog(): void {
    const dialogData: UserFormDialogData = {
      ...
      roles: [Role.Admin, Role.SuperAdmin, Role.Agents] // Allowed roles to create here
    };
  ...
  openEditDialog(user: User): void {
    const dialogData: UserFormDialogData = {
      mode: 'edit',
      user: user,
      roles: [Role.Admin, Role.SuperAdmin, Role.Agents]
    };
  ...
  handleUpdateUser(request: UserUpdateRequest, file?: File) {
    ...
    // Note: UserFormDialog usually handles image upload for existing users inside the dialog itself for better UX,
    // but if we are passing it back, we handle it here. 
    // In my implementation of UserFormDialog, I handled upload inside the dialog for EDIT mode. 
    // So 'file' might be null here if already uploaded.
